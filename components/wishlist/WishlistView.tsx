...
function WishlistInner({
  ...
  function onToggle(item: WishItem) {
    ...
    setList((l) =>
      l.map((i) => (i.id === item.id ? { ...i, done: !i.done } : i)),
    );
  ...
  return (
    <div className="mx-auto w-full max-w-xl">
      <div className="paper-texture relative rounded-sm bg-[#f7f0e0] p-6 shadow-2xl shadow-black/25 md:p-9">
        ...
        <p className={`mt-1 font-hand-alt text-base ${theme.inkSoft}`}>
          {list.length === 0
            ...
            : remaining === 0
              ? "Semua kelakon! Waktunya nulis yang baru 🎉"
      ...
      <div className="mt-6 flex items-center justify-between">
        <Link
          href={`/book/${slug}`}
          className={`font-hand text-lg opacity-60 transition hover:opacity-100 ${theme.inkSoft}`}
        >
          ← Kembali ke buku
        </Link>
        <MusicToggle className={theme.inkSoft} />
      </div>
    </div>
  );
}
